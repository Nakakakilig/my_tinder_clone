...
from domain.exceptions import SwipeAlreadyExistsError, SwipeCreateError, SwipeNotFoundError
...
class SwipeRepositoryImpl(ISwipeRepository):
    ...
    async def create_swipe(self, swipe: Swipe) -> Swipe:
        try:
            if swipe.profile_id_1 > swipe.profile_id_2:
                swipe.profile_id_1, swipe.profile_id_2 = swipe.profile_id_2, swipe.profile_id_1
            if await self.get_swipe_by_two_profile_ids(swipe.profile_id_1, swipe.profile_id_2):
                raise SwipeAlreadyExistsError(swipe.profile_id_1, swipe.profile_id_2)
        ...
        except SwipeCreateError as e:
            raise SwipeCreateError(swipe.profile_id_1, swipe.profile_id_2) from e
    ...
    async def get_swipes(self, limit: int, offset: int) -> list[Swipe] | None:
        try:
            ...
            raise SwipeNotFoundError() from e
    ...
    async def get_swipes_by_profile_id(
        ...
        try:
            stmt = (
                select(SwipeORM)
                .where(
                    (SwipeORM.profile_id_1 == profile_id) | (SwipeORM.profile_id_2 == profile_id)
                )
            ...
            raise SwipeNotFoundError(profile_id) from e
    ...
    async def get_swipe_by_two_profile_ids(
        ...
        try:
            if profile_id_1 == profile_id_2:
                raise SwipeNotFoundError(profile_id_1, profile_id_2)
            if profile_id_1 > profile_id_2:
            ...
            raise SwipeNotFoundError(profile_id_1, profile_id_2) from e
